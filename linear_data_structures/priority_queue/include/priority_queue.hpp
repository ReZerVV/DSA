...
private:
    void heapify(const size_t size, const size_t index) {
        int largest = index;
        int left_child = 2 * index + 1;
        int right_child = 2 * index + 2;
        
        if (left_child < size && _compare(_buffer[left_child], _buffer[largest])) {
            largest = left_child;
        }
    
        if (right_child < size && _compare(_buffer[right_child], _buffer[largest])) {
            largest = right_child;
        }

...
// Not movable constructos.
    hash_table(const hash_table &other) {
        ...
        delete[] _buffer;
        _buffer = new node_type[_capacity];
        for (size_t index = 0; index < other._capacity; ++index) {
            if (other._buffer[index]._state) {
                size_t hindex = _hash(other._buffer[index]._key) % _capacity;
                if (!_buffer[hindex]._state) {
                    _buffer[hindex] = other._buffer[index];
                } else {
                    hindex = _step_hash(other._buffer[index]._key) % _capacity;
                    do {
                        if (!_buffer[hindex]._state) {
                            _buffer[hindex] = other._buffer[index];
                            break;
